            ...
            if (result) {
                ...
                pool.query(data, function (err, result1) {
                    ...
                    if (result1 != null && result1.length >0) {
                        ...
                        res.json({ status: true, result: result1,message:'Record insert successfully' });
...
function saveItemFunc(invoiceId, itemData, callback) {
    ...
    itemData.forEach(element => {
        ...
        var insItem = "insert into 	tbl_item(hsnCode, itemName, quantity, itemPrice, unit, total, discount, discountAmount, subTotal, gst, cgst, sgst, igst, cgstAmount, sgstAmount, igstAmount, taxAmount, netTotal, invoiceId, companyId, isActive) values ('" + element.hsnCode + "','" + element.itemName + "','" + element.quantity + "','" + element.itemPrice + "','" + element.unit + "','" + element.total + "','" + element.discount + "','"+element.discountAmount+"','" + element.subTotal + "','" + element.gst + "','" + element.cgst + "','" + element.sgst + "','" + element.igst + "','" + element.cgstAmount + "','" + element.sgstAmount + "','" + element.igstAmount + "','"+element.taxAmount+"','" + element.netTotal + "','" + invoiceId + "', '"+element.companyId+"',1)";
        pool.query(insItem, function (err, insResData) {
        });
    });
